...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      ...
      } catch (err) { console.error("Admin fetch requests failed", err); }
    };
    fetchRequests();
  ...
  const handleAiSuggest = async () => {
    ...
    try {
      const res = await axios.post('/api/ai-suggest', { priority: activePriority, assets: placedAssets });
      setAiSuggestion({ text: res.data.suggestion, action: "RE-ROUTE" });
    } catch (err) {
      setAiSuggestion({ text: "Strategy engine error. Manual override suggested.", action: "RE-ROUTE" });
    }
  };
  ...
  const handleAskAdvisor = async () => {
    ...
    setIsBroadcasting(true); // Using broadcasting state as loader

    try {
      const res = await axios.post('/api/policy-advisor', { query: q });
      setAdvisorLog(p => [...p, { role: 'ai', content: res.data.report }]);
    } catch (err) {
      setAdvisorLog(p => [...p, { role: 'ai', content: "Communication with Nexus engine lost. Please check local Ollama status." }]);
    }
    setIsBroadcasting(false);
  };

  const handleBroadcastPolicy = async () => {
    if (!policyForm.policy.trim()) return;
    setIsBroadcasting(true);
    try {
      // Simulate backend broadcast
      await new Promise(resolve => setTimeout(resolve, 1500));
      alert("GLOBAL DIRECTIVE BROADCASTED: " + policyForm.policy);
    } catch (err) {
